...
SELECT first_name,last_name,COUNT(*) FROM owners JOIN vehicles ON vehicles.owner_id=owners.id GROUP BY owners.id ORDER BY first_name;
...
SELECT first_name,last_name,AVG(price) AS average_price, COUNT(*) FROM owners JOIN vehicles ON vehicles.owner_id = owners.id 
GROUP BY owners.id HAVING COUNT(*) > 1 AND AVG(price) > 10000 ORDER BY first_name DESC;